...
For at se et overblik over afhængighederne mellem store moduler (pakker) i PC Applikation, se figur \ref{pakke} nedenfor. Der blev identificeret behovet for en GUI (AutoSonographyWPF), indhente en 3D scanning fra 3D kamera (ComputerVisionLibrary), beregne positioner og rotationer på baggrund af en 3D scanning (CalculationLibrary), samt at sende positurer til Robotarm (RoboLibrary). De forskellige biblioteker anvender fælles datastrukturer, disse blev samlet i ét bibliotek for at undgå cykliske forbindelser (DataStructures).
