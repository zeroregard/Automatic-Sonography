...
/// <summary>
/// Mostly made by Microsoft, based on their Kinect Fusion Explorer which showcases most features of the Kinect API and Fusion library
/// https://msdn.microsoft.com/en-us/library/dn193975.aspx 
/// </summary>
/// 
namespace ComputerVisionLibrary
{
    public class KinectFusionizer : IDisposable, IKinect
    {
        private bool CaptureCurrent;
        private KinectSensor sensor;

        private object volumeLock = new object();
        private object rawDataLock = new object();
        public const int FramesToCapture = 64;
        private int FramesCaptured = 0;

        private float voxelsPerMeter = 384;
        private int integrationWeight = 1000;
        private int voxelsX = 256;
        private int voxelsY = 256;
        private int voxelsZ = 256;

        private float minDepthClip = 1.20f;
        private float maxDepthClip = 1.66f;
        ...
        public KinectFusionizer(ComputerVisionMaster master)
        {
            this.master = master;
        ...
        private void SetUpKinectVariables()
        {
            sensor = KinectSensor.GetDefault();
            if (sensor == null)
            {
                throw new Exception("The Kinect could not be found"); //TODO: Show an error in the GUI instead
            }
            sensor.Open();
        ...
        {
            MapColorToDepth();
            volume.IntegrateFrame(depthFloatFrame, resampledColorFrameDepthAligned, integrationWeight, FusionDepthProcessor.DefaultColorIntegrationOfAllAngles, worldToCameraTransform);
        }
